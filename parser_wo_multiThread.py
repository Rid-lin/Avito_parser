...
def get_html(try_url, proxy, retry=5):
    """
    Аналог requests.get(url, proxies)
    Возвращает ответ с возможностью повторить запрос несколько раз.
    При этом комментирует свои действия.
    :param try_url: URL для отдачи
    :param proxy: Proxy для использования
    :param retry: количество повторений
    :return:
    """
    print('Пытаюсь получить страницу -', try_url, end='')
    while retry:
        try:
...
def get_raw_table(items):
    rows_table = []
    # page = get_html(url, proxy)
    # if page is None: return  # Если страница не получена то выходим с возвратом None
    # if page.text.find('По вашему запросу ничего не найдено') >= 0 : return  # Если на странице получаем ошибку поиска то выходим с возвратом None
    # doc = html.document_fromstring(page.content)
    # items = doc.cssselect('div.js-catalog_before-ads .item_table')  # отсекаем всЁ до нужного нам раздела перед рекламой
    # items.extend(doc.cssselect('div.js-catalog_after-ads .item_table'))  # добавляем всЁ до нужного нам раздела после рекламы
    for item in items:
...
def get_table(url, proxy, pages):
    ...
    for i in range(1, pages + 1):
        ...
        page = get_html(next_url, proxy)
        if page is None: return  # Если страница не получена то выходим с возвратом None
        if page.text.find(
                'По вашему запросу ничего не найдено') >= 0: return  # Если на странице получаем ошибку поиска то выходим с возвратом None
        doc = html.document_fromstring(page.content)
        items = doc.cssselect(
            'div.js-catalog_before-ads .item_table')  # отсекаем всЁ до нужного нам раздела перед рекламой
        items.extend(
            doc.cssselect('div.js-catalog_after-ads .item_table'))  # добавляем всЁ до нужного нам раздела после рекламы
        raw_page = get_raw_table(items)
        if not raw_page:
            print("Страница пустая. Заканчиваем", '\n')
            return project
        project.extend(raw_page)
    return project
